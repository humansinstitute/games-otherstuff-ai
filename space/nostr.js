 ...
 */
async function publishGameResult(score, gameUrl, gameName = 'Snake') {
  const player = NostrPlayer.get();
  if (!player) throw new Error('No player initialized');

  const content = `I scored ${score} sats playing ${gameName}!

Play here: ${gameUrl}

#SpaceInvaders #nostr #gaming`;
