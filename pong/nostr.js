 ...
 */
async function publishGameResult(score, gameUrl, gameName = 'Pong', levelReached = 1) {
  const player = NostrPlayer.get();
  if (!player) throw new Error('No player initialized');

  const content = `I scored ${score} points playing ${gameName} and reached level ${levelReached}!
