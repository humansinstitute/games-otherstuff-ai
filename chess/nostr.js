...
/**
 * Publish chess game result to Nostr
 * @param {string} result - The game result (win/loss/draw)
 * @param {number} moves - Number of moves in the game
 * @param {string} playerColor - Color the player played as
 */
async function publishGameResult(result, moves, playerColor = 'white') {
  const player = NostrPlayer.get();
  if (!player) throw new Error('No player initialized');

  const gameUrl = 'https://chesstr.otherstuff.ai';

  let resultText;
  if (result === 'win') {
    resultText = `I won against the computer in ${moves} moves playing as ${playerColor}!`;
  } else if (result === 'loss') {
    resultText = `I lost to the computer after ${moves} moves playing as ${playerColor}.`;
  } else if (result === 'draw') {
    resultText = `I drew against the computer after ${moves} moves playing as ${playerColor}.`;
  } else {
    resultText = `Chess game ended after ${moves} moves.`;
  }
