 ...
 * @param {string} gameResult.levelName - Name of the level
 * @param {number} gameResult.levelNumber - Level number (1-indexed)
 * @param {number} gameResult.saved - Lemmings saved
 * @param {number} gameResult.total - Total lemmings
 * @param {number} gameResult.time - Time in seconds
 * @param {boolean} gameResult.won - Whether the level was won
 * @param {string} gameUrl - URL to the game
 */
async function publishGameResult(gameResult, gameUrl) {
  const player = NostrPlayer.get();
  if (!player) throw new Error('No player initialized');
  ...
  const content = `Level ${levelNumber}: ${levelName} ${statusEmoji} ${statusText}!
