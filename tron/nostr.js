 ...
 * @param {boolean} isVictory - Whether the player just won
 */
async function publishGameResult(wins, gameUrl, gameName = 'Tron Bike', isVictory = false) {
  const player = NostrPlayer.get();
  if (!player) throw new Error('No player initialized');

  const message = isVictory
    ? `I derezzed my opponent in ${gameName}! Total wins: ${wins}`
    : `Racing light cycles in ${gameName}! Total wins: ${wins}`;

  const content = `${message}

Play here: ${gameUrl}

#TronBike #nostr #gaming #lightcycle`;
